...
export const setDownloadedEpisodeLimit = async (podcastId: string, limit?: number) => {
  ...
  await AsyncStorage.setItem(PV.Keys.DOWNLOADED_EPISODE_LIMITS, JSON.stringify(downloadedEpisodeLimits))
}

export const setAllDownloadedEpisodeLimits = async (episodeLimits: any) => {
  if (episodeLimits) {
    await AsyncStorage.setItem(PV.Keys.DOWNLOADED_EPISODE_LIMITS, JSON.stringify(episodeLimits))
  } else {
...
export const setDownloadedEpisodeLimitGlobalDefault = async (bool: boolean) => {
  if (bool) {
    await AsyncStorage.setItem(PV.Keys.DOWNLOADED_EPISODE_LIMIT_GLOBAL_DEFAULT, JSON.stringify(bool))
  } else {
    await AsyncStorage.removeItem(PV.Keys.DOWNLOADED_EPISODE_LIMIT_GLOBAL_DEFAULT)
  }
}
...
export const updateAllDownloadedEpisodeLimitDefaults = async (bool: boolean) => {
  const userInfo = await getAuthenticatedUserInfoLocally()
  const subscribedPodcastIds = userInfo[0] ? userInfo[0].subscribedPodcastIds : []
  const limits = {}
  for (const id of subscribedPodcastIds) {
    limits[id] = bool
  }

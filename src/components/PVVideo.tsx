...
import { addParameterToURL, convertUrlToSecureHTTPS, encodeSpacesInString } from '../lib/utility'
...
export class PVVideo extends React.PureComponent<Props, State> {
         ...
         componentWillUnmount() {
           ...
           PVEventEmitter.removeListener(
             PV.Events.PLAYER_VIDEO_NEW_EPISODE_ITEM_LOADED,
             this._handleNewEpisodeItemShouldLoad
           )
           ...
           PVEventEmitter.removeListener(
             PV.Events.PLAYER_VIDEO_LIVE_GO_TO_CURRENT_TIME,
             this._handleGoToLiveCurrentTime
           )
         }
                 ...
                 () => {
                   ;(async () => {
                     try {
                       ...
                       const { Authorization, filePath, isDownloadedFile } = await videoGetDownloadedFileInfo(
                         nowPlayingItem
                       )
